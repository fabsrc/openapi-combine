...
const combineInfo = (
  infoObjects: OpenAPIV3.InfoObject[]
...
const combineServers = (
  serversObjects: OpenAPIV3.ServerObject[][]
): OpenAPIV3.ServerObject[] => R.flatten(serversObjects);

const combinePaths = (
  pathObjects: OpenAPIV3.PathsObject[]
): OpenAPIV3.PathsObject => mergeDeepAll(pathObjects);
...
const combineTags = (
  tagsObjects: OpenAPIV3.TagObject[][]
): OpenAPIV3.TagObject[] => R.flatten(tagsObjects);
...
export const combine = (schemas: OpenAPIV3.Document[]): OpenAPIV3.Document => {
  ...
  document.externalDocs = combineExternalDocs(
    pluckIfExists("externalDocs", schemas)
  );
